...
def upgrade():
    ...
    result = list(
        conn.execute(
            "SELECT page.copy_id, page.number FROM page " "GROUP BY page.copy_id, page.number HAVING COUNT(*) > 1"
        )
    )
    if result:
        raise RuntimeError(
            "There are duplicate page entries, not able to run migration.\r\n"
    ...
    result = list(
        conn.execute(
            "SELECT submission.exam_id, copy.number FROM copy "
    ...
    if result:
        raise RuntimeError(
            "There are duplicate copy entries, not able to run migration.\r\n"
