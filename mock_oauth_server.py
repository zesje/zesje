...
def authorize():
    ...
    return redirect(f"{query_params['redirect_uri'][0]}?code=test&state={query_params['state'][0]}")


@app.route("/user", methods=["GET"])
def user():
    """Returns user information"""
    return dict(email=app.config["OWNER_OAUTH_ID"], name=app.config["OWNER_NAME"])
