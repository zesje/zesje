...
def add_test_data(app):
    ...
    problem_widget_1 = ProblemWidget(
        id=1, name="problem widget", problem_id=1, page=2, width=100, height=150, x=40, y=200, type="problem_widget"
    )
...
def test_add_templated_exam(datadir, test_client):
    with open(os.path.join(datadir, "blank-a4-2pages.pdf"), "rb") as pdf:
        response = test_client.post(
            "/api/exams", data={"exam_name": "The Exam", "pdf": pdf, "layout": ExamLayout.templated.name}
        )
...
def test_add_templated_exam_without_pdf(datadir, test_client):
    response = test_client.post("/api/exams", data={"exam_name": "The Exam", "layout": ExamLayout.templated.name})

    assert response.status_code == 409


def test_add_unstructured_exam(test_client):
    response = test_client.post("/api/exams", data={"exam_name": "The Exam", "layout": ExamLayout.unstructured.name})
    assert response.status_code == 200
...
def test_add_exam_invalid_layout(test_client):
    response = test_client.post("/api/exams", data={"exam_name": "The Exam", "layout": -1})

    assert response.status_code == 422


@pytest.mark.parametrize("name, status_code", [("New name", 200), ("", 422)], ids=["New name", "Empty"])
...
def test_get_exams_mult_choice(test_client, add_test_data):
    mc_option_1 = {"x": 100, "y": 40, "problem_id": 1, "label": "a", "name": "test"}
    test_client.put("/api/mult-choice/", json=mc_option_1)

    mc_option_2 = {"x": 100, "y": 40, "problem_id": 1, "label": "a", "name": "test"}
...
@pytest.mark.parametrize("finalized, status_code", [(False, 200), (True, 409)], ids=["Not finalized", "Finalized"])
