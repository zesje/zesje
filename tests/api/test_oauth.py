...
def callback_request(login_client, login_app):
    ...
    with requests_mock.Mocker() as m:
        ...
        m.get(
            login_app.config["OAUTH_INFO_URL"],
            json={
                login_app.config["OAUTH_ID_FIELD"]: login_app.config["OWNER_OAUTH_ID"],
...
@pytest.mark.parametrize(
    "url, expected",
    [("", "/"), ("/graders", "/graders"), ("http://evil.com/page", "/page")],
...
def test_internal_grader(login_app, login_client):
    db.session.add(Grader(name="internal", oauth_id="grader_123456789", internal=True))
    db.session.commit()
