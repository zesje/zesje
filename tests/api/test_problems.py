...
def add_test_data(app):
    for layout in ExamLayout:
        ...
        feedback_option = FeedbackOption(
            id=id * 10, problem_id=id, text="text", description="desc", score=1, parent_id=problem.root_feedback.id
        )
...
@pytest.mark.parametrize(
    "id, status", [(1, 200), (2, 200), (42, 404)], ids=["Exists Templated", "Exists unstructured", "Not exists"]
)
...
@pytest.mark.parametrize(
    "id, new_name, status",
    [(1, "New", 200), (2, "New", 200), (42, "New", 404), (1, "   ", 422)],
...
def test_new_problem_has_root_fo(test_client, add_test_data):
    req_body = {"exam_id": 2, "x": 0, "y": 0, "width": 400, "height": 200, "page": 1, "name": "Problem"}
    result = test_client.post("/api/problems", json=req_body)
