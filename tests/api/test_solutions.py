...
def add_test_data(app):
    ...
    for i in range(2):
        fo_parent = FeedbackOption(
            id=i + 1, problem=problem, text=chr(i + 65), description="", score=i, parent=root, mut_excl_children=i == 1
        )
...
def test_add_remark(test_client, add_test_data, monkeypatch_current_user):
    ...
    res = test_client.post("/api/solution/1/1/1", json={"remark": remark})
...
def test_toggle_feedback(test_client, add_test_data, monkeypatch_current_user, parent_id):
    # toogle parent
    for j in range(2):
        res = test_client.put("/api/solution/1/1/1", json={"id": parent_id})
    ...
    # toogle child
    for j in range(2):
        res = test_client.put("/api/solution/1/1/1", json={"id": parent_id * 10 + 2})
        assert res.status_code == 200
    ...
    res = test_client.put("/api/solution/1/1/1", json={"id": parent_id * 10 + 1})
    assert res.status_code == 200
    checked = res.get_json()["state"]
    assert checked
...
def test_approve(test_client, add_test_data, monkeypatch_current_user):
    ...
    res = test_client.put("/api/solution/1/1/1", json={"id": 1})

    # toogle approve
    for j in range(2):
        res = test_client.put("/api/solution/approve/1/1/1", json={"approve": j == 0})
...
@pytest.mark.parametrize("id, status_code", [(1, 200), (11, 200)], ids=["Children", "No children"])
...
def test_add_exclusive_options(test_client, add_test_data):
    ...
    res = test_client.put("/api/solution/1/1/1", json={"id": 2 * 10 + 1})
    ...
    res = test_client.put("/api/solution/1/1/1", json={"id": 2 * 10 + 2})
    ...
    res = test_client.put("/api/solution/1/1/1", json={"id": 1 * 10 + 2})
    res = test_client.get("/api/solution/1/1/1")
    checked_feedback = res.get_json()["feedback"]
