from zesje.api.submissions import _shuffle_submissions
...
def test_shuffle_same_grader():
    ...
    assert _shuffle_submissions(submissions, 0) == _shuffle_submissions(submissions, 0)
    # Assert that shuffled is not the same order as unshuffled, but has the same elements
    assert _shuffle_submissions(submissions, 0) != submissions and \
        set(submissions) == set(_shuffle_submissions(submissions, 0))
...
def test_shuffle_different_grader():
    ...
    submissions0 = _shuffle_submissions(submissions, 0)
    submissions1 = _shuffle_submissions(submissions, 1)
