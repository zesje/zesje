...
def mock_solution_data(monkeypatch, datadir):
    def mock_return(exam_id, student_id):
        student = {"id": 1, "first_name": "Jane", "last_name": "Doe", "email": "jane.doe@fake.come", "total": 8}

        results = [{"id": 1, "name": "name", "max_score": 10, "score": 8, "feedback": [], "remarks": ""}]
...
@pytest.mark.parametrize("exam_id, status", [(1, 200), (6, 404)], ids=["OK", "Invalid exam"])
...
@pytest.mark.parametrize(
    "exam_id, template, status",
    [(1, default_email_template, 200), (6, default_email_template, 404), (1, "Not valid {% if false }", 400)],
...
@pytest.mark.parametrize(
    ...
    [("{{student.first_name} {{student.last_name}}", 400), ("{{teacher.address}}", 400)],
