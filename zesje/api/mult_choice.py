...
class MultipleChoice(MethodView):
    ...
    def put(self, args):
        ...
        if problem.exam.finalized:
            return (
                dict(
                    status=405,
                    message="Cannot create multiple choice option and corresponding feedback option"
                    + " in a finalized exam.",
                ),
                405,
            )
        ...
        new_feedback_option = FeedbackOption(
            problem_id=problem.id, text=label, description="", score=0, parent=problem.root_feedback
        )
        ...
        return (
            dict(
                status=200,
                mult_choice_id=mc_entry.id,
                feedback_id=new_feedback_option.id,
    ...
    def delete(self, mc_option):
        ...
        return (
            dict(
                status=200,
                mult_choice_id=mc_option.id,
                feedback_id=mc_option.feedback_id,
                message=f"Multiple choice question with id {mc_option.id} deleted."
                + f"Feedback option with id {mc_option.feedback_id} deleted.",
            ),
            200,
        )
