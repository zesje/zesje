...
class Students(MethodView):
    ...
    def post(self, csv):
        ...
        if len(errors) == len(results):
            message = (
                "All the rows failed to process, "
...
def _row_to_student(row):
    try:
        ...
        content = dict(
            id=int(str(row["OrgDefinedId"]).replace("#", "")),
        ...
        if student_same_mail:
            if student_same_mail.id == student.id:
                ...
                return Result.ERROR, (
                    ...
                    other_id=student_same_mail.id,
                    other_first=student_same_mail.first_name,
                    other_last=student_same_mail.last_name,
                )
