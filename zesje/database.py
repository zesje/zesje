...
db = SQLAlchemy(model_class=declarative_base(cls=Model, metaclass=NoNameMeta, name="Model", metadata=meta))
...
ExamLayout = enum.Enum("ExamLayout", "templated unstructured")
...
class Exam(db.Model):
    ...
    widgets = db.relationship("ExamWidget", backref="exam", cascade="all", order_by="ExamWidget.id", lazy=True)
    finalized = Column(Boolean, default=False, server_default="0")
    grade_anonymous = Column(Boolean, default=False, server_default="0")
    layout = Column(
        "layout", Enum(ExamLayout), server_default="templated", default=ExamLayout.templated, nullable=False
    )
...
class Submission(db.Model):
    ...
    solutions = db.relationship(
        "Solution", backref="submission", cascade="all", order_by="Solution.problem_id", lazy=True
    )
    copies = db.relationship("Copy", backref="submission", cascade="all", order_by="Copy.number", lazy=True)
...
class Page(db.Model):
    ...
    def retrieve(cls, copy, page_number):
        ...
        return cls.query.filter(cls.copy == copy, cls.number == page_number).one_or_none() or cls(
            copy=copy, number=page_number
        )
...
"""
GradingPolicy = enum.Enum("GradingPolicy", "set_nothing set_blank set_single")
...
class Problem(db.Model):
    ...
    grading_policy = Column(
        ...
        default=GradingPolicy.set_blank,
        nullable=False,
    )
    feedback_options = db.relationship(
        "FeedbackOption", backref="problem", cascade="all", order_by="FeedbackOption.id", lazy=True
    )
...
class FeedbackOption(db.Model):
    ...
    mc_option = db.relationship(
        "MultipleChoiceOption", backref=backref("feedback", cascade="all"), cascade="all", uselist=False, lazy=True
    )
...
class ExamWidget(Widget):
    ...
    __mapper_args__ = {"polymorphic_identity": "exam_widget"}
    ...
    def size(self):
        if self.name == "student_id_widget":
            ...
            return (
                (digits + 1) * (fontsize + margin) + 4 * margin + text_box_width,
...
class ProblemWidget(Widget):
    ...
    __mapper_args__ = {"polymorphic_identity": "problem_widget"}
