...
def dump(config, database=None, create_backup_file=False, file_name=None):
    ...
    if create_backup_file:
        if not file_name:
            file_name = Path(config["DATA_DIRECTORY"]) / "mysql_backup_{}.sql".format(
...
def _default_options(datadir):
    return (
        f"--defaults-file={_defaults_file()} --datadir={datadir} --basedir=$CONDA_PREFIX/bin "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "action", choices=["create", "start", "stop", "is-running", "backup"], help="Action to be performed"
    )
    parser.add_argument("-i", action="store_true", help="Run commant in interactive mode, attached to the console.")
    parser.add_argument("--allow-running", action="store_true", help="Allow the MySQL server to be already running.")
    parser.add_argument("--allow-exists", action="store_true", help="Allow MySQL to be initialized already.")

    parser.add_argument("--output", type=str, help="Output file when running the backup action, optional")
